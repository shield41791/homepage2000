...
2000년대 초반(2000~2004년경) 개인 홈페이지 감성을 재현한 가상 인물 "소이"(세일러문·웨딩피치 등 소녀 만화/애니메이션을 좋아하는 10대)의 개인 홈페이지. 레퍼런스는 `References/Girlish` 폴더의 스크린샷(Sugar Cubes, ChibiUsa's Palace, Oh Pooh, Hello Kitty 연하장 페이지, Open Love Project 등 — 파스텔톤 소녀 감성 지오시티즈류 홈페이지들). 브라우저 크롬(익스플로러 UI)은 참고 대상에서 제외하고 웹페이지 자체 디자인만 반영. 게시판 표는 `References/Bulma`(bulma07~08) 참고.
...
- 인물 설정: 세일러문·웨딩피치·카드캡터 사쿠라 등 소녀 만화·애니메이션 덕후 10대. 굿즈 수집, 코스프레, 순정만화, 팬아트가 주요 소재.
...
- 상단: 레인보우 스트라이프 배너 안에 버블 타이틀 로고("소이의 달빛 다이어리") + 도메인 서브텍스트(`http://moonprism.wo.to`).
...
- **Entrance** — 대문 이미지 placeholder + 버블 타이틀 + 환영 문구(세일러문·웨딩피치 언급) + ENTER 버튼 + 방문자 카운터(LCD 스타일 흑색 박스) + BGM 안내.
- **Home** — 소개글(애니 덕후 자기소개) + 기울어진 사진 + 세일러문 굿즈 소개 + 게시판/공지 최신글 미리보기 + 사이드바 위젯.
- **Profile** — 사진, 자기소개, 관심사(순정만화 그리기, 굿즈 모으기 등), 좋아하는 것(세일러문/웨딩피치/카드캡터 사쿠라), 스냅샷, SNS/연락처.
- **Notice** — NO/SUBJECT/DATE/HIT 목록, 클릭 시 `PostView.dc.html?board=notice&id=`.
- **Board** — 카테고리 탭(자유/일기/여행기) + NO/SUBJECT/NAME/DATE/HIT 목록. 글 내용은 애니 정주행/굿즈/순정만화 소재.
- **Photos** — 탭(ALL/사진/일러스트/웹툰/스크린샷) + 표 형태 썸네일 그리드. 코스프레 사진, 팬아트, 명장면 캡처 등.
- **Videos** — 표 형태 썸네일 그리드(재생 아이콘 오버레이) + 클릭 시 다이얼로그(실제 재생 없음). 변신 장면/OP 모음 등.
- **Projects** — 2열 그리드, 클릭 시 새 탭에서 외부 링크. 세일러문 팬사이트, 만화 동아리 사이트 등.
